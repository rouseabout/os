...
qemu-efi: efi/EFI/BOOT/BOOTX64.EFI efi/kernel efi/initrd
	qemu-system-x86_64 $(QEMUFLAGS) -bios /usr/share/ovmf/OVMF.fd -drive file=fat:rw:efi,format=raw

efi-cdrom.iso: efi/EFI/BOOT/BOOTX64.EFI efi/kernel efi/initrd
	dd if=/dev/zero of=$@ bs=1M count=6
	mformat -i $@ ::
	mmd -i $@ EFI
	mmd -i $@ EFI/BOOT
	mcopy -i $@ efi/EFI/BOOT/BOOTX64.EFI ::EFI/BOOT
	mcopy -i $@ efi/kernel efi/initrd ::

qemu-efi-cdrom: efi-cdrom.iso
	qemu-system-x86_64 $(QEMUFLAGS) -bios /usr/share/ovmf/OVMF.fd -cdrom $<

efi/EFI/BOOT/BOOTX64.EFI: loader/efiloader.c
	x86_64-w64-mingw32-gcc -o loader/efiloader.o -c loader/efiloader.c -Wall -ffreestanding -I/usr/include/efi/
	x86_64-w64-mingw32-gcc -o $@ loader/efiloader.o -nostdlib -Wl,-dll -shared -Wl,--subsystem,10 -e efi_main
