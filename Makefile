...
kernel.bin: kernel/linker.ld $(KERNEL_OBJS) .toolchain-$(ARCH)-stage1
	$(LD) $(LDFLAGS) -o $@ -T kernel/linker.ld $(KERNEL_OBJS) -lgcc
	$(STRIP) --strip-all $@
...
clean:
	rm -f kernel.bin $(KERNEL_OBJS) cdrom.iso iso/boot/kernel.bin initrd iso/boot/initrd disk_image disk_image.vdi hd/boot/kernel.bin hd/boot/initrd $(TFTP_FILES) programs/*.o libc/*.o libdl/*.o libg/*.o libm/*.o $(PROGRAMS) libc.a libdl.a libg.a libm.a $(TEST_BIN) $(DUMPELF_BIN) $(DUMPEXT2_BIN) $(EXT2TEST_BIN) .sysroot
	rm -rf sysroot
